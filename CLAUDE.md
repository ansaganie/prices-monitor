...
- `.github/workflows/monitor.yml` — `workflow_dispatch` (manual) + `repository_dispatch` (`external-cron`), needs `permissions: contents: write` to read/write the state release (the same permission already used for releases, no new secret). The `repository_dispatch` payload can carry `client_payload.skipAvailabilityAlert` to mute the availability-count half of the Change Alert from cron-job.org's side without a redeploy — wired to `SKIP_AVAILABILITY_ALERT` and read by `check.js`'s `isAvailabilityAlertSuppressed()`; see README's "Guaranteeing ≥1 run per working-hour".
...
**The availability-count half of the Change Alert can be muted at runtime, without a redeploy.** `SKIP_AVAILABILITY_ALERT` (sourced from the external-cron payload — see the `monitor.yml` bullet above) makes `isChangeAlert()` ignore a count-only change and makes `buildReport()` withhold the 🔥 count-crossing marker; the Reported Baseline still updates every run regardless, so unmuting later doesn't dump a backlog — only a genuinely new change alerts. Price Change Alerts, the Daily Digest, and the Run-Gap Watchdog are unaffected.
