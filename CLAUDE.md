...
**BI Group API failures never fail the Actions run.** `src/check.js` catches `fetchAllPlacements()` failures per object, keeps that object's previous numbers, and reports the error inline in the Telegram scan report (`⚠️ Данные недоступны: <message>`) instead of exiting non-zero. The Actions run only fails on something the script genuinely can't recover from or report (e.g. Telegram itself unreachable).
